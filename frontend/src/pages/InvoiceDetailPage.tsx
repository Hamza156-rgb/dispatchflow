...
export default function InvoiceDetailPage() {
  ...
  const handleDownloadPdf = () => {
    downloadPdf.mutate({ id: id!, invoiceNumber: invoice.invoiceNumber });
  };
  ...
  return (
    <div style={{ padding: 28 }}>
      ...
      {/* Breadcrumb */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 24, fontSize: 13, color: 'var(--color-muted)' }}>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 24, flexWrap: 'wrap' }}>
        ...
        {(invoice.status === 'SENT' || invoice.status === 'OVERDUE') && (
          <Button onClick={() => { setPayForm(f => ({ ...f, amount: String(invoice.totalAmount) })); setPayModal(true); }} variant="success">
            💳 Record Payment
          </Button>
        )}
        <Button onClick={() => setEmailModal(true)} variant="secondary">📧 Send Email</Button>
        <Button onClick={handleDownloadPdf} loading={downloadPdf.isPending} variant="secondary">⬇️ Download PDF</Button>
        <Button onClick={handleDelete} variant="danger">🗑️ Delete</Button>
      </div>

      {/* Invoice card */}
      <div style={{ maxWidth: 780, background: 'var(--color-bg)', borderRadius: 16, border: '1.5px solid var(--color-border)', overflow: 'hidden' }}>
        {/* Header */}
        <div style={{ background: '#0f172a', padding: '28px 36px', display: 'flex', justifyContent: 'space-between' }}>
          <div>
        ...
        <div style={{ padding: '28px 36px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 24, marginBottom: 28 }}>
            {[
              ...
              ['Status',     null],
              ['Total',      `$${Number(invoice.totalAmount).toLocaleString('en-US', { minimumFractionDigits: 2 })}`],
            ].map(([label, val]) => (
              <div key={label as string}>
                <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--color-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 6 }}>{label}</div>
                {label === 'Status' ? <StatusBadge status={invoice.status} /> :
                 label === 'Total' ? <div style={{ fontSize: 18, fontWeight: 900, color: '#0f172a' }}>{val}</div> :
          ...
          <div style={{ background: 'var(--color-surface)', borderRadius: 10, padding: '16px 20px', marginBottom: 28 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--color-muted)', textTransform: 'uppercase', marginBottom: 8, letterSpacing: '0.06em' }}>Bill To</div>
            ...
            <thead>
              <tr style={{ background: 'var(--color-surface)' }}>
                {['Description', 'Qty', 'Rate', 'Amount'].map(h => (
                  <th key={h} style={{ padding: '10px 14px', textAlign: h === 'Description' ? 'left' : 'right',
            ...
            <div style={{ width: 280 }}>
              {[['Subtotal', `$${Number(invoice.subtotal).toFixed(2)}`], [`Tax (${Number(invoice.taxRate)}%)`, `$${Number(invoice.taxAmount).toFixed(2)}`]].map(([l, v]) => (
                <div key={l} style={{ display: 'flex', justifyContent: 'space-between',
                  padding: '6px 0', fontSize: 13, color: 'var(--color-muted)', borderBottom: '1px solid var(--color-border)' }}>
                  <span>{l}</span><span>{v}</span>
                </div>
              ))}
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 8,
                padding: '12px 16px', background: '#0f172a', borderRadius: 8, fontSize: 16, fontWeight: 900, color: '#fff' }}>
                <span>Total Due</span>
          ...
          {invoice.notes && (
            <div style={{ marginTop: 24, padding: '14px 18px', background: 'var(--color-surface)', borderRadius: 8 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--color-muted)', textTransform: 'uppercase', marginBottom: 6 }}>Notes</div>
      ...
      {invoice.payments && invoice.payments.length > 0 && (
        <div style={{ maxWidth: 780, marginTop: 20, background: 'var(--color-bg)', borderRadius: 14, border: '1.5px solid var(--color-border)', overflow: 'hidden' }}>
      ...
      <Modal open={payModal} onClose={() => setPayModal(false)} title="Record Payment" width={460}>
        <div style={{ textAlign: 'center', marginBottom: 24 }}>
          <div style={{ fontSize: 36, fontWeight: 900, color: '#16a34a' }}>${Number(invoice.totalAmount).toFixed(2)}</div>
          <div style={{ color: 'var(--color-muted)', fontSize: 14 }}>Total due for {invoice.invoiceNumber}</div>
        </div>
        ...
        <Button onClick={handleRecordPayment} loading={recordPayment.isPending} style={{ width: '100%', marginTop: 8 }}>
          ✅ Confirm Payment
        </Button>
      </Modal>
      ...
      <Modal open={emailModal} onClose={() => setEmailModal(false)} title="Send Invoice via Email" width={460}>
        ...
        <Button onClick={handleSendEmail} loading={sendEmail.isPending} style={{ width: '100%' }}>
          📧 Send Invoice
        </Button>
      </Modal>
    </div>
